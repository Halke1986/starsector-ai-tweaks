...
class BallisticsKtTest {
    ...
    fun testWillHitBounds() {
        val bounds = MockBoundsAPI("getOrigSegments" to listOf(
        ...
        val target = MockShipAPI("getLocation" to Vector2f(1659.4774f, -449.50232f), "getVelocity" to Vector2f(52.902084f, -105.472f), "getCollisionRadius" to 35.755074f, "getFacing" to 219.58746f, "getExactBounds" to bounds, "getMutableStats" to MockMutableShipStatsAPI("getTimeMult" to MutableStat(1f)))

        val actual = weapon.ballistics.willHitBounds(target, BallisticParams(1f, 0f))
    ...
    fun testTargetFasterThanProjectile() {
        ...
        assertTrue(ballistics.intercept(target, BallisticParams(1f, 0f)).length >= approachesInfinity)
        assertEquals(Arc(0f, weapon.currAngle), ballistics.interceptArc(target, BallisticParams(1f, 0f)))
        assertTrue(ballistics.closestHitRange(target, BallisticParams(1f, 0f)) >= approachesInfinity)
        assertNull(ballistics.willHitCircumference(target, BallisticParams(1f, 0f)))
    }
    ...
    fun testWeaponInsideTargetRadius() {
        ...
        assertTrue(ballistics.canTrack(target, BallisticParams(1f, 0f)))
        assertNotNull(ballistics.intercept(target, BallisticParams(1f, 0f)))
        assertEquals(Arc(360f, 0f), ballistics.interceptArc(target, BallisticParams(1f, 0f)))
        assertEquals(0f, ballistics.closestHitRange(target, BallisticParams(1f, 0f)))
        assertNotNull(ballistics.willHitCircumference(target, BallisticParams(1f, 0f)))
    }
}

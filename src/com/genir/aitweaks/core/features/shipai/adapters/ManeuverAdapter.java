...
public class ManeuverAdapter implements ManeuverInterface {
    ...
    ManeuverAdapter(Ship ship, Ship target, Vector2f location) {
        try {
            ...
            Class<?> loaderManager = scriptLoader.loadClass("com.genir.aitweaks.launcher.CoreLoaderManager");
            MethodType getCoreLoaderType = MethodType.methodType(ClassLoader.class);
            MethodHandle getCoreLoader = MethodHandles.lookup().findVirtual(loaderManager, "get", getCoreLoaderType);
            ClassLoader coreLoader = (ClassLoader) getCoreLoader.invoke(loaderManager.newInstance());

            // Load Maneuver class.
            Class<?> m = coreLoader.loadClass("com.genir.aitweaks.core.features.shipai.ai.Maneuver");

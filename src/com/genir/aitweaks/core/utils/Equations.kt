package com.genir.aitweaks.core.utils

import com.genir.aitweaks.core.utils.types.LinearMotion
import org.lwjgl.util.vector.Vector2f
import kotlin.math.sqrt

enum class Solution {
    SMALLER_NON_NEGATIVE,
    LARGER_NON_NEGATIVE,
}

/**
 * Solve the following cosine law equation for t:
 *
 * a(t)² = b(t)² + r² - 2*b(t)*r*cosA
 *
 * where
 *
 * a(t) = |P + V * t|
 * b(t) =  q + w * t
 *
 * Equation can be expanded in the following way:
 * (Px + Vx * t)²          + (Py + Vy * t)²          = (q + w * t)²        + r² - 2((q + w * t) * r * cosA)
 * Px² + 2Px*Vx*t + Vx²*t² + Py² + 2Py*Vy*t + Vy²*t² = q² + 2q*w*t + w²*t² + r² - 2q*r*cosA - 2w*t*r*cosA
 * 0 = (Vx² + Vy² - w²)*t² + 2(Px*Vx + Py*Vy - w*q + r*w*cosA)*t + (Px² + Py² - q² - r² + 2q*r*cosA)
 */
fun solve(p: Vector2f, v: Vector2f, q: Float, w: Float, r: Float, cosA: Float, solution: Solution): Float {
    val a = (v.x * v.x) + (v.y * v.y) - (w * w)
    val b = (p.x * v.x) + (p.y * v.y) - (w * q) + (w * r * cosA)
    val c = (p.x * p.x) + (p.y * p.y) - (r * r) - (q * q) + (2 * q * r * cosA)

    return quad(a, 2 * b, c, solution)
}

/** Time after which point P travelling with velocity V
 * will find itself at R distance from (0,0). */
fun solve(p: Vector2f, v: Vector2f, r: Float, solution: Solution): Float {
    val a = (v.x * v.x) + (v.y * v.y)
    val b = (p.x * v.x) + (p.y * v.y)
    val c = (p.x * p.x) + (p.y * p.y) - (r * r)

    return quad(a, 2 * b, c, solution)
}

fun solve(pv: LinearMotion, q: Float, w: Float, r: Float, cosA: Float, solution: Solution): Float {
    return solve(pv.position, pv.velocity, q, w, r, cosA, solution)
}

fun solve(pv: LinearMotion, r: Float, solution: Solution): Float {
    return solve(pv, 0f, 0f, r, 0f, solution)
}

/** Solve the quadratic equation [ax² + bx + c = 0] for x. */
private fun quad(a: Float, b: Float, c: Float, solution: Solution): Float {
    // Equation is degenerated to const case.
    if (a == 0f && b == 0f) {
        return Float.NaN
    }

    // Equation is degenerated to linear case.
    if (a == 0f) {
        val x = -c / b
        return selectSolution(x, x, solution)
    }

    val delta = b * b - 4 * a * c

    // Equation has no real solutions.
    if (delta < 0) {
        return Float.NaN
    }

    val d = sqrt(delta)
    val x1 = (-b + d) / (2 * a)
    val x2 = (-b - d) / (2 * a)

    return selectSolution(x1, x2, solution)
}

private fun selectSolution(x1: Float, x2: Float, solution: Solution): Float {
    // Equation has no positive solutions.
    if (x1 < 0 && x2 < 0) {
        return Float.NaN
    }

    // x1 is the only positive solution.
    if (x2 < 0) {
        return x1
    }

    // x2 is the only positive solution.
    if (x1 < 0) {
        return x2
    }

    return if (solution == Solution.SMALLER_NON_NEGATIVE) {
        minOf(x1, x2)
    } else {
        maxOf(x1, x2)
    }
}